...
const props = defineProps(["show", "id", "hidePencil", "hideDelete", "deleteFunc"])
...
<template>
  ...
  <div class="color-container">
    ...
    <label class="pencil" :for="'edit-' + props.id">
      <svg :fill="textColor" xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24"><path d="M7.127 22.562l-7.127 1.438 1.438-7.128 5.689 5.69zm1.414-1.414l11.228-11.225-5.69-5.692-11.227 11.227 5.689 5.69zm9.768-21.148l-2.816 2.817 5.691 5.691 2.816-2.819-5.691-5.689z"/></svg></label>
    <input class="hidden" :id="'edit-' + props.id" type="checkbox" v-model="showInputs">
    </div>
    <div v-show="!props.hideDelete">
      <label class="delete" :for="'delete-' + props.id">
        <svg width="2em" height="2em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M5 5L19 19M5 19L19 5" :stroke="textColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </label>
      <input class="hidden" :id="'delete-' + props.id" type="checkbox" @click="props.deleteFunc">
    </div>
  </div>
